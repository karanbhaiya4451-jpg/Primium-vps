...
# style="primary" = Blue | style="primary" = Green | style="primary" = Red
...
if not TOKEN:
    sys.exit("❌ BOT_TOKEN missing! Please add your token.")
...
def main_menu_kb(admin: bool = False) -> types.InlineKeyboardMarkup:
    ...
    if admin:
        kb.add(Btn(f"Aᴅᴍɪɴ Pᴀɴᴇʟ", callback_data="menu_admin", style="primary"))  # Blue kar diya

    return kb

def back_main_kb() -> types.InlineKeyboardMarkup:
    return types.InlineKeyboardMarkup().add(
        Btn(f"{G['back']}  Mᴀɪɴ Mᴇɴᴜ", callback_data="menu_main", style="primary"))
...
def back_kb(target: str, label: str = "Back") -> types.InlineKeyboardMarkup:
    return types.InlineKeyboardMarkup().add(
        Btn(f"{G['back']}  {sc(label)}", callback_data=target, style="primary"))
...
def payments_kb(plan: Optional[str] = None) -> types.InlineKeyboardMarkup:
    ...
    for k, v in PAYMENT_METHODS.items():
        kb.add(Btn(f"{v['tag']}  {sc(v['name'])}", callback_data=f"pay_{k}{suffix}", style="primary"))
...
def admin_kb() -> types.InlineKeyboardMarkup:
    ...
    kb.add(
        ...
        Btn(f"{G['wallet']}  Pᴀʏᴍᴇɴᴛꜱ",     callback_data="adm_payments", style="primary"),
    )
    kb.add(
        Btn(f"{G['broadcast']}  Bʀᴏᴀᴅᴄᴀꜱᴛ", callback_data="adm_broadcast",style="primary"),
        Btn(f"{G['no']}  Bᴀɴ / Uɴʙᴀɴ",      callback_data="adm_ban",      style="primary"),
    )
    kb.add(
        Btn(f"{G['plus']}  Gɪᴠᴇ Pʟᴀɴ",      callback_data="adm_giveplan", style="primary"),
        Btn(f"{G['ok']}  Aᴘᴘʀᴏᴠᴇ Pᴀʏ",      callback_data="adm_approve",  style="primary"),
    )
    kb.add(
    ...
    kb.add(
        ...
        Btn(f"{G['lock']}  Sᴇᴄᴜʀɪᴛʏ",       callback_data="adm_security", style="primary"),
    )
    kb.add(
        Btn(f"{G['warn']}  Mᴀɪɴᴛᴇɴᴀɴᴄᴇ",    callback_data="adm_maint",    style="primary"),
    ...
    kb.add(
        Btn(
            ...
            style="primary" if appr_on else "danger"),
        Btn(
    ...
    kb.add(
        ...
        Btn(f"{G['refresh']}  Fᴏʀᴄᴇ Bᴀᴄᴋᴜᴘ", callback_data="adm_force_backup", style="primary"),
    )
    ...
    kb.add(
        ...
        Btn("🛡️  Sᴇᴄ Cᴇɴᴛᴇʀ",      callback_data="adm_sec_center",     style="primary"),
    )
    kb.add(
        Btn("💬  Nᴏᴛɪꜰɪᴄᴀᴛɪᴏɴꜱ",   callback_data="adm_notify_center",  style="primary"),
    ...
    kb.add(
        ...
        Btn("💳  Pᴀʏ Cᴏɴꜰɪɢ",      callback_data="adm_pay_config",     style="primary"),
    )
    kb.add(
    ...
    kb.add(
        Btn("🔗  Rᴇꜰᴇʀʀᴀʟ Sʏꜱ",    callback_data="adm_referral_sys",   style="primary"),
        Btn("🧹  Jᴀɴɪᴛᴏʀ",          callback_data="adm_janitor",        style="primary"),
    )
    kb.add(
    ...
    kb.add(
        Btn("⏱️  Rᴀᴛᴇ Lɪᴍɪᴛꜱ",      callback_data="adm_rate_config",    style="primary"),
        Btn("📡  Lɪᴠᴇ Mᴏɴɪᴛᴏʀ",      callback_data="adm_live_monitor",   style="primary"),
    )
    kb.add(
        Btn("💎  Rᴇᴠ Gᴏᴀʟꜱ",        callback_data="adm_rev_goals",      style="primary"),
    ...
    kb.add(
        ...
        Btn("🔐  Adᴍɪɴ 2FA",         callback_data="adm_admin_2fa",      style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Mᴀɪɴ Mᴇɴᴜ", callback_data="menu_main", style="primary"))
    return kb
...
def github_kb(status: Dict[str, Any]) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn(f"{G['plus']}  Bᴀᴄᴋᴜᴘ Nᴏᴡ",      callback_data="gh_backup_now",  style="primary"))
    ...
    kb.add(Btn(
        ...
        style="primary" if status["autoEnabled"] else "danger"))
    kb.add(
    ...
    kb.add(Btn(f"{G['no']}  Cʟᴇᴀʀ Cᴏɴꜰɪɢ", callback_data="gh_clear",     style="primary"))
...
def bot_actions_kb(bot_id: str, running: bool, premium: bool = False) -> types.InlineKeyboardMarkup:
    ...
    if running:
        kb.add(
            Btn(f"{G['stop']}  Sᴛᴏᴘ",       callback_data=f"bot_stop_{bot_id}",    style="primary"),
            Btn(f"{G['refresh']}  Rᴇꜱᴛᴀʀᴛ", callback_data=f"bot_restart_{bot_id}", style="primary"),
        )
    else:
        kb.add(
            Btn(f"{G['play']}  Sᴛᴀʀᴛ",      callback_data=f"bot_start_{bot_id}",   style="primary"),
            Btn(f"{G['refresh']}  Rᴇꜱᴛᴀʀᴛ", callback_data=f"bot_restart_{bot_id}", style="primary"),
        )
    kb.add(
    ...
    if premium:
        ...
        kb.add(Btn(f"{glyph}  {label}", callback_data=f"bot_tunnel_{bot_id}",
                   style="primary" if is_open else "success"))
    kb.add(Btn(f"{G['arrow']}  Dᴏᴡɴʟᴏᴀᴅ", callback_data=f"bot_dl_{bot_id}", style="primary"))
    kb.add(Btn(f"{G['no']}  Dᴇʟᴇᴛᴇ",       callback_data=f"bot_delete_{bot_id}", style="primary"))
...
def confirm_kb(yes_cb: str, no_cb: str = "menu_main", yes_label: str = "Confirm",
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  {sc(yes_label)}", callback_data=yes_cb, style="primary"),
        Btn(f"{G['no']}  {sc(no_label)}",  callback_data=no_cb,  style="primary"),
    )
    return kb
...
GH = {
    "token": "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",
    "repo": "karanbhaiya4451-jpg/Primum-vps",
    "branch": "main",
    "intervalMin": 360,
    "autoEnabled": True,
}
...
def render_bots_menu(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{G['plus']}  {sc('Upload')}",   callback_data="menu_upload", style="primary"),
...
def render_adm_maintenance(call: types.CallbackQuery) -> None:
    ...
    kb.add(Btn(
        ...
        style="primary" if cur else "success"))
    kb.add(Btn(
...
def render_adm_settings(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{G['refresh']}  {sc('Reload Caches')}",
            callback_data="adm_set_reload",  style="primary"),
        Btn(f"{G['eye']}  {sc('System Info')}",
            callback_data="adm_set_sysinfo", style="primary"),
    )
    kb.add(
        Btn(f"{G['refresh']}  {sc('Restart All Bots')}",
            callback_data="adm_set_restart_all", style="primary"),
        Btn(f"{G['no']}  {sc('Stop All Bots')}",
            callback_data="adm_set_stop_all",    style="primary"),
    )
    kb.add(
        Btn(f"{G['warn']}  {sc('Clean Orphans')}",
            callback_data="adm_set_clean_orphans", style="primary"),
...
def render_adm_confirm_custom(call: types.CallbackQuery, action: str,
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  {sc('Yes')}",    callback_data=action,  style="primary"),
...
def render_adm_user_tools(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn("💰  Wᴀʟʟᴇᴛ Aᴅᴊᴜꜱᴛ",  callback_data="adm_wallet_admin",    style="primary"),
    ...
    kb.add(
        ...
        Btn("🔄  Rᴇꜱᴇᴛ Uꜱᴇʀ",     callback_data="adm_user_reset",      style="primary"),
    )
    kb.add(
        Btn("🎁  Gɪᴠᴇ Pʟᴀɴ",      callback_data="adm_giveplan",        style="primary"),
        Btn("🚫  Bᴀɴ/Uɴʙᴀɴ",      callback_data="adm_ban",             style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_bot_manager(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("💥  Cʀᴀꜱʜᴇᴅ Bᴏᴛꜱ",     callback_data="adm_crashed_bots",        style="primary"),
        Btn("🔄  Rᴇꜱᴛᴀʀᴛ Sᴛᴏᴘᴘᴇᴅ",   callback_data="adm_mass_restart_stopped", style="primary"),
    )
    kb.add(
    ...
    kb.add(
        Btn("🔴  Kɪʟʟ Aʟʟ Nᴏᴡ",     callback_data="adm_kill_all_now",        style="primary"),
        Btn("🗑️  Cʟᴇᴀɴ Oʀᴘʜᴀɴꜱ",    callback_data="adm_set_clean_orphans",   style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_mass_restart_stopped(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  {sc('Yes, Start All')}", callback_data="adm_mass_restart_stopped_yes", style="primary"),
...
def render_adm_sec_center(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("📋  Tʜʀᴇᴀᴛ Lᴏɢ",      callback_data="adm_threat_log",     style="primary"),
        Btn("📊  Sᴇᴄ Sᴛᴀᴛꜱ",        callback_data="adm_sec_stats",      style="primary"),
    )
    kb.add(
        Btn("✅  Wʜɪᴛᴇʟɪꜱᴛ Uꜱᴇʀ",  callback_data="adm_sec_whitelist",  style="primary"),
        Btn("🚫  Bʟᴀᴄᴋʟɪꜱᴛ",        callback_data="adm_sec_blacklist",  style="primary"),
    )
    kb.add(
...
def render_adm_notify_center(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("📢  Nᴏᴛɪꜰʏ Eᴠᴇʀʏᴏɴᴇ", callback_data="adm_notify_all",        style="primary"),
    ...
    kb.add(
        ...
        Btn("📣  Qᴜɪᴄᴋ Aɴɴᴏᴜɴᴄᴇ",  callback_data="adm_quick_announce",     style="primary"),
    )
    kb.add(
        Btn("📡  Bʀᴏᴀᴅᴄᴀꜱᴛ",        callback_data="adm_broadcast",          style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_sys_tools(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("🧹  Cʟᴇᴀʀ Cᴀᴄʜᴇ",   callback_data="adm_clear_cache",     style="primary"),
    )
    kb.add(
    ...
    kb.add(
        Btn("🔄  Rᴇʟᴏᴀᴅ Cᴀᴄʜᴇ",  callback_data="adm_set_reload",      style="primary"),
...
def render_adm_gh_browser(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    if has_token:
        kb.add(Btn("📂  Bʀᴏᴡꜱᴇ Mʏ Rᴇᴘᴏꜱ",  callback_data="adm_gh_repos",        style="primary"))
        if has_repo:
...
def render_adm_gh_file_view(call: types.CallbackQuery, repo: str, path: str) -> None:
    ...
    def _bg() -> None:
        ...
        kb = types.InlineKeyboardMarkup(row_width=2)
        if runnable:
            kb.add(Btn("▶️  Rᴜɴ Aꜱ Bᴏᴛ",  callback_data="adm_gh_run_file",  style="primary"))
        kb.add(
...
def render_adm_pay_config(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'✅' if auto_approve else '❌'}  Aᴜᴛᴏ-Aᴘᴘʀ",
            ...
            style="primary" if auto_approve else "danger"),
    ...
    kb.add(
        Btn("✅  Aᴘᴘʀᴏᴠᴇ Pᴀʏ",     callback_data="adm_approve",          style="primary"),
        Btn("📤  Exᴘᴏʀᴛ CSV",       callback_data="adm_user_export_csv",  style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_pay_method_edit(call: types.CallbackQuery, key: str) -> None:
    ...
    kb.add(
        Btn(f"{'❌ Disable' if enabled else '✅ Enable'}",
            ...
            style="primary" if enabled else "success"),
        Btn("✏️  Cʜᴀɴɢᴇ Nᴜᴍʙᴇʀ",
...
def render_adm_pay_receipt_tmpl(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("🔄  Rᴇꜱᴇᴛ",     callback_data="adm_tmpl_reset_payment_received", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Pᴀʏ Cᴏɴꜰɪɢ", callback_data="adm_pay_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bot_cfg(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("🔐  Eɴᴠ Sᴛʀɪᴘ",        callback_data="adm_bc_env",       style="primary"),
    )
    kb.add(
...
def render_adm_bc_env(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'✅' if strip else '❌'}  Sᴛʀɪᴘ Sᴇᴄʀᴇᴛꜱ",
            ...
            style="primary" if strip else "danger"),
...
def render_adm_bc_sandbox(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'✅' if wipe else '❌'}  Fɪʟᴇ Wɪᴘᴇ",
            ...
            style="primary" if wipe else "danger"),
    ...
    kb.add(
        Btn(f"{'✅' if net else '❌'}  Nᴇᴛᴡᴏʀᴋ",
            ...
            style="primary" if net else "danger"),
    )
    kb.add(Btn(f"{G['back']}  Bᴏᴛ Cᴏɴꜰɪɢ", callback_data="adm_bot_cfg", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bc_policy(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'✅' if auto_r else '❌'}  Aᴜᴛᴏ-Rᴇꜱᴛᴀʀᴛ",
            ...
            style="primary" if auto_r else "danger"),
    ...
    kb.add(
        ...
        Btn(f"{'✅' if auto_dg else '❌'}  Aᴜᴛᴏ-Dɢ",
            ...
            style="primary" if auto_dg else "danger"),
    )
    kb.add(Btn(f"{G['back']}  Bᴏᴛ Cᴏɴꜰɪɢ", callback_data="adm_bot_cfg", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_app_emojis(call: types.CallbackQuery) -> None:
    ...
    kb.add(Btn("🔄  Rᴇꜱᴇᴛ Aʟʟ", callback_data="adm_app_emoji_reset", style="primary"))
    kb.add(Btn(f"{G['back']}  Aᴘᴘᴇᴀʀᴀɴᴄᴇ", callback_data="adm_appearance", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("appearance", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_app_banner(call: types.CallbackQuery) -> None:
    ...
    kb.add(Btn("🔄  Rᴇʙᴜɪʟᴅ Bᴀɴɴᴇʀꜱ", callback_data="adm_rebuild_banners", style="primary"))
    kb.add(Btn(f"{G['back']}  Aᴘᴘᴇᴀʀᴀɴᴄᴇ", callback_data="adm_appearance", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("appearance", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_coupon_plus(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("➕  Cʀᴇᴀᴛᴇ Cᴏᴜᴘᴏɴ",   callback_data="adm_coupons",          style="primary"),
    ...
    kb.add(
        Btn("🗑️  Cʟᴇᴀʀ Exᴘɪʀᴇᴅ",   callback_data="adm_coupon_clearexp",  style="primary"),
...
def render_adm_coupon_expiry(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("🗑️  Cʟᴇᴀʀ Exᴘɪʀᴇᴅ", callback_data="adm_coupon_clearexp", style="primary"))
...
def render_adm_janitor(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn("▶️  Rᴜɴ Nᴏᴡ",         callback_data="adm_jan_run_now",   style="primary"),
...
def render_adm_webhooks(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("❌  Cʟᴇᴀʀ (Pᴏʟʟɪɴɢ)", callback_data="adm_wh_clear", style="primary"),
    )
    kb.add(
...
def render_adm_feature_flags(call: types.CallbackQuery) -> None:
    ...
    kb.add(Btn("🔄  Rᴇꜱᴇᴛ Aʟʟ Fʟᴀɢꜱ", callback_data="adm_ff_reset_all", style="primary"))
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_rate_config(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'✅ ON' if global_rl else '❌ OFF'}  Gʟᴏʙᴀʟ RL",
            ...
            style="primary" if global_rl else "danger"),
    )
...
def render_adm_scheduler(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("➕  Aᴅᴅ Tᴀꜱᴋ",      callback_data="adm_sched_add",  style="primary"),
...
def render_adm_sched_list(call: types.CallbackQuery) -> None:
    ...
    for t in tasks[:8]:
        ...
        kb.add(
            ...
            Btn(f"🗑️ {tid[:8]}",
                callback_data=f"adm_sched_del_{tid}",    style="primary"),
        )
    kb.add(Btn("➕  Aᴅᴅ Tᴀꜱᴋ",     callback_data="adm_sched_add",   style="primary"))
...
def render_adm_import_export(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("📤  Exᴘᴏʀᴛ Cᴏɴꜰɪɢ",  callback_data="adm_export_full_cfg", style="primary"),
    ...
    kb.add(
        Btn("♻️  Fᴀᴄᴛᴏʀʏ Rᴇꜱᴇᴛ",  callback_data="adm_import_reset",    style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_admin_2fa(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    if not secret:
        kb.add(Btn("🔑  Sᴇᴛᴜᴘ 2FA", callback_data="adm_2fa_setup", style="primary"))
    else:
        kb.add(
            Btn(f"{'✅ ON' if enabled else '❌ OFF'}  Tᴏɢɢʟᴇ",
                callback_data="adm_bc_toggle_admin_2fa_enabled",
                style="primary" if enabled else "danger"),
            Btn("🗑️  Dɪꜱᴀʙʟᴇ+Rᴇꜱᴇᴛ", callback_data="adm_2fa_disable", style="primary"),
        )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_leaderboard(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("💰  Tᴏᴘ Sᴘᴇɴᴅᴇʀꜱ",   callback_data="adm_lb_spenders",  style="primary"),
...
def render_adm_bot_controls_panel(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn("🔴  Kɪʟʟ Aʟʟ",         callback_data="adm_kill_all_now", style="primary"),
        Btn("🔄  Rᴇꜱᴛᴀʀᴛ Sᴛᴏᴘᴘᴇᴅ",  callback_data="adm_mass_restart_stopped", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_bc_single(call: types.CallbackQuery, bid: str) -> None:
    ...
    if is_running:
        kb.add(
            Btn("⏹  Sᴛᴏᴘ",       callback_data=f"adm_bc_stop_{bid[:20]}",    style="primary"),
            Btn("🔄  Rᴇꜱᴛᴀʀᴛ",   callback_data=f"adm_bc_restart_{bid[:20]}", style="primary"),
        )
    else:
        kb.add(Btn("▶️  Sᴛᴀʀᴛ",   callback_data=f"adm_bc_restart_{bid[:20]}", style="primary"))
    kb.add(
    ...
    kb.add(
        ...
        Btn("🗑️  Dᴇʟᴇᴛᴇ",        callback_data=f"adm_bc_del_{bid[:20]}",     style="primary"),
    )
...
def render_adm_subscriptions(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("⏰  Exᴘɪʀɪɴɢ Sᴏᴏɴ",    callback_data="adm_sub_expiring",      style="primary"),
        Btn("❌  Exᴘɪʀᴇᴅ",           callback_data="adm_sub_expired",        style="primary"),
    )
    kb.add(
        Btn("📨  Rᴇᴍɪɴᴅ Aʟʟ",       callback_data="adm_sub_remind_all",     style="primary"),
    ...
    kb.add(
        Btn(f"{'✅' if auto_dg else '❌'}  Aᴜᴛᴏ-Dɢ",
            callback_data="adm_sub_auto_downgrade",
            style="primary" if auto_dg else "danger"),
        Btn("⚡  Rᴜɴ Dᴏᴡɴɢʀᴀᴅᴇ",    callback_data="adm_sub_run_downgrade",  style="primary"),
    )
    kb.add(
...
def render_adm_payment_requests(call):
    ...
    kb  = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("✅  Aᴘᴘʀᴏᴠᴇ", callback_data="adm_pay_approve_select", style="primary"),
        Btn("❌  Rᴇᴊᴇᴄᴛ",   callback_data="adm_pay_reject_select",  style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_api_keys(call):
    ...
    lines.append(G["div"] + FOOTER)
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(Btn("🗑️  Rᴇᴠᴏᴋᴇ Aʟʟ", callback_data="adm_apikey_revoke_all", style="primary"))
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def render_adm_export_menu(call):
    ...
    kb.add(
        ...
        Btn("📥  Iᴍᴘᴏʀᴛ DB",  callback_data="adm_import_db",        style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("import_export", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_tg_channel_backup(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("\U0001f4be  B\u1d00\u1d04\u1d0b\u1d1c\u1d18 N\u1d0f\u1d21", callback_data="adm_tg_bkp_now", style="primary"),
    )
    kb.add(
        Btn(f"{'OK' if auto_on else 'OFF'}  A\u1d1c\u1d1b\u1d0f", callback_data="adm_tg_bkp_toggle_auto",
            style="primary" if auto_on else "danger"),
        Btn("\U0001f4e5  R\u1d07\u02e2\u1d1b\u1d0f\u0280\u1d07", callback_data="adm_tg_bkp_restore", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  A\u1d05\u1d0d\u026a\u0274", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_gh_repo_host_menu(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("\U0001f5d1\ufe0f  R\u1d07\u1d0d\u1d0f\u1d20\u1d07 R\u1d07\u1d18\u1d0f", callback_data="gh_host_remove_sel", style="primary"),
    )
...
def render_adm_approval_group(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'OK' if grp_on else 'OFF'}  Verify", callback_data="adm_grpv_toggle",
            style="primary" if grp_on else "danger"),
        Btn("+ Add Group", callback_data="adm_grpv_add", style="primary"),
    )
    kb.add(
        Btn("- Remove Group", callback_data="adm_grpv_remove", style="primary"),
...
def render_adm_private_group_panel(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        ...
        Btn("Clear",        callback_data="adm_apgrp_clear",  style="primary"),
    )
    kb.add(
        Btn(f"Notify: {'Admins' if notify_admins_only else 'All'}", callback_data="adm_apgrp_toggle_notify", style="primary"),
        Btn("Test",         callback_data="adm_apgrp_test",   style="primary"),
    )
...
def render_bot_delete_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  Delete Record", callback_data=f"bot_delyes_{bot_id}", style="primary"),
...
def render_bot_delfiles_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  Delete Files", callback_data=f"bot_delfilesyes_{bot_id}", style="primary"),
        Btn(f"{G['no']}  Cancel",        callback_data=f"bot_view_{bot_id}",       style="primary"),
    )
    show_text(call.message.chat.id, cap, kb, call=call)
...
def render_bot_delall_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['no']}  Delete All", callback_data=f"bot_delalyes_{bot_id}", style="primary"),
...
def render_adm_security(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("\U0001f6e1\ufe0f  Sec Center",       callback_data="adm_sec_center",    style="primary"),
...
def render_adm_maintenance(call: types.CallbackQuery) -> None:
    ...
    kb.add(Btn(f"{'Disable' if on else 'Enable'} Maintenance",
               ...
               style="primary" if on else "success"))
    kb.add(Btn(f"{G['back']}  Admin", callback_data="menu_admin"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_settings(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn("\U0001f451  Transfer Owner",    callback_data="adm_set_owner",       style="primary"),
        Btn("\U0001f527  Maint Mode",        callback_data="adm_maint",           style="primary"),
    )
    kb.add(
        Btn("\U0001f504  Restart All Bots",  callback_data="adm_set_restart_all", style="primary"),
        Btn("\U0001f534  Stop All Bots",     callback_data="adm_set_stop_all",    style="primary"),
    )
    kb.add(
    ...
    kb.add(
        ...
        Btn("\U0001f504  Reload Caches",     callback_data="adm_set_reload",      style="primary"),
    )
    kb.add(
...
def render_adm_github(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn(f"{'OK' if auto else 'OFF'}  Auto", callback_data="gh_toggle_auto",
            style="primary" if auto else "danger"),
        Btn("\U0001f4be  Backup Now", callback_data="gh_backup_now", style="primary"),
    )
    kb.add(
        Btn("\U0001f4e5  Restore",    callback_data="gh_restore_now", style="primary"),
...
def render_adm_confirm(call: types.CallbackQuery, action: str, label: str) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  Yes, do it",   callback_data=f"{action}_yes", style="primary"),
...
def render_adm_confirm_custom(call: types.CallbackQuery, action: str,
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['ok']}  Yes", callback_data=action,  style="primary"),
